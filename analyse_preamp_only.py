...
def plot_samples_scatter(data):
    ...
    plt.savefig('plots/preamp_only/samples_scatter_plot.svg', format='svg')
    plt.close()
...
def plot_peak_height_histogram(max_samples_list, show_info=False, info_list=None, voltage="NaN"):
    ...
    plt.savefig(f'plots/preamp_only/peak_height_histogram_{voltage}.svg', format='svg')
    plt.close()
...
def plot_compare_peak_height_hists(ph_bins, voltages):
    ...
    plt.savefig('plots/preamp_only/peak_height_bins_comparision.svg', format='svg')
...
def main():
    ...
    for root, dirs, files in os.walk("data/preamp_only"):
        for dir in dirs:
            subdirectory_path = os.path.join(root, dir)
            file_path = os.path.join(subdirectory_path, f'FILTERED/0@DT5720B #2-3-316_Data_{subdirectory_path.split("preamp_only/")[1]}.csv')
