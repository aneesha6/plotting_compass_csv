...
def plot_energy_histogram(data_dict, show_info=False, info_list=None, voltage="NaN"):
    ...
    plt.savefig(f'plots/low_pressure/e_long_gate_histogram_{voltage}.svg', format='svg')
    plt.close()
...
def plot_samples_scatter(data):
    ...
    plt.savefig('plots/low_pressure/samples_scatter_plot.svg', format='svg')
    plt.close()
...
def plot_peak_height_histogram(max_samples_list, show_info=False, info_list=None, voltage="NaN"):
    ...
    # Save the histogram as an SVG file
    plt.savefig(f'plots/low_pressure/peak_height_histogram_{voltage}.svg', format='svg')
    plt.close()
...
def plot_compare_energy_hists(e_bins, voltages):
    ...
    plt.savefig('plots/low_pressure/energy_bins_comparision.svg', format='svg')
    plt.close()
...
def plot_compare_peak_height_hists(ph_bins, voltages):
    ...
    plt.savefig('plots/low_pressure/peak_height_bins_comparision.svg', format='svg')
...
def main():
    ...
    for root, dirs, files in os.walk("data/low_pressure"):
        for dir in dirs:
            subdirectory_path = os.path.join(root, dir)
            file_path = os.path.join(subdirectory_path, f'FILTERED/0@DT5720B #2-3-316_Data_{subdirectory_path.split("low_pressure/")[1]}.csv')
